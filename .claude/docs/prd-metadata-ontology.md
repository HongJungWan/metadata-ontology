...
> **역할**: 도메인 용어와 물리 스키마를 잇는 공유 메타데이터 계층. 자연어 질의를 물리 컬럼·코드값으로 매핑하고, LLM에 줄 스키마 설명을 만들어 잘못된 해석을 막는다.
> **스택**: Java 21 · Spring Boot 3.5.x · JPA · QueryDSL · PostgreSQL · Caffeine(소비자 캐시)
...
같은 개념을 팀마다 다르게 부른다. "상품쿠폰"을 누구는 "상쿠"라 하고, 물리 테이블에는 `code_type='PRODUCT_COUPON'`으로 들어가 있다. 이 간극을 메우지 않으면 SQL 검색 계층과 LLM이 서로 다른 걸 가리키고, 답이 어긋난다. 이 서비스는 그 사이를 잇는 단일 기준(SSOT)을 둔다.
...
- 질의에 맞는 스키마 설명을 LLM에 먼저 줘서 **제멋대로 해석하는 걸 막는다.**
...
### 1.3 규모(초기)
- 도메인 용어 약 180개, 물리 스키마 매핑 약 150건.
...
기존 컨벤션을 따른다: `BaseEntity` 상속, `@Where(clause="deleted_at IS NULL")` 소프트삭제, `@Builder`, 한글 Javadoc, `@Data` 금지. 패키지는 `com.hris.medi.domain.<도메인>`, 조회는 QueryDSL 커스텀 리포지토리.

```mermaid
erDiagram
    TERM ||--o{ TERM_SCHEMA_MAP : maps
    SCHEMA_MAPPING ||--o{ TERM_SCHEMA_MAP : mapped_by
    TERM ||--o{ SYNONYM : has
    SCHEMA_MAPPING }o--|| SCHEMA_CATALOG : references
    SCHEMA_CATALOG ||--o{ CODE_VALUE : defines
    SQL_PATTERN }o--|| TERM : triggers
```
...
> `Term`과 `SchemaMapping`은 다대다(조인 엔티티 `TermSchemaMap`). 한 용어가 여러 컬럼에, 한 컬럼이 여러 용어에 걸린다.
...
| `GET /api/terms/{name}/mappings` | term | 물리 컬럼·코드값 매핑 | 용어 단건 조회 |
...
| `POST /api/prompt-context` | `query`/`terms` | LLM에 줄 스키마 설명 블록 | 잘못된 해석 방지(§6) |
...
- 새 매핑·변경은 검토 후 활성화한다(`status`).
- 팀별 용어·동의어·매핑을 시트/CSV로 한 번에 올린다.
- 사전이 바뀌면 P1 캐시를 무효화하도록 신호를 보낸다(버전 헤더 또는 이벤트).
...
이렇게 주면 LLM이 정의된 매핑 안에서만 답하고, 컬럼 의미를 임의로 지어내지 않는다.
...
- 대표 질의셋으로 **동의어·정규화 적용 전후의 재현율을 비교한다.**
- 매핑을 쓴 검색과 안 쓴 검색(원문 키워드)을 나란히 놓고, P1의 채점 점수와 함께 본다.
