...
import com.hris.metadata.domain.term.entity.SynonymType;
...
class ExpansionServiceTest {
    ...
    private Synonym synonym(String surface, String canonical, SynonymType type) {
    ...
    void expandsSynonyms() {
        lenient().when(synonymRepository.findBySurfaceWithTerm("세틀"))
                .thenReturn(Optional.of(synonym("세틀", "정산상태", SynonymType.ABBREVIATION)));
        lenient().when(synonymRepository.findBySurfaceWithTerm("머천트"))
                .thenReturn(Optional.of(synonym("머천트", "가맹점", SynonymType.COLLOQUIAL)));
