package com.hris.metadata.domain.promptcontext;
...
import com.hris.metadata.domain.schema.repository.SchemaCatalogRepository;
...
public class PromptContextService {
    ...
    private String build(List<Term> terms) {
        List<UuidTermId> termIds = terms.stream()
                .map(term -> new UuidTermId(term.getTermId()))
                .toList();
    ...
    private void appendSchemaSection(StringBuilder builder, List<UuidTermId> termIds) {
        List<ColumnMappingRow> rows = schemaMappingRepository.findColumnMappingsByTermIds(
                termIds.stream().map(UuidTermId::value).toList());

        Set<String> renderedTables = new LinkedHashSet<>();
        for (ColumnMappingRow row : rows) {
            if (renderedTables.add(row.getPhysicalTable())) {
                builder.append("테이블: ").append(row.getPhysicalTable()).append("\n");
            }
            appendColumnLine(builder, row);
        }
    }

    private void appendColumnLine(StringBuilder builder, ColumnMappingRow row) {
        Optional<SchemaCatalog> catalog = schemaCatalogRepository
                .findByPhysicalTableAndPhysicalColumn(row.getPhysicalTable(), row.getPhysicalColumn());
        builder.append("- ").append(row.getPhysicalColumn());
    ...
    /** termId 매핑 헬퍼 (디미터 법칙 준수용 단순 래퍼) */
    private record UuidTermId(java.util.UUID value) {
    }
}
